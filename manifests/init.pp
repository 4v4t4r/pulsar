...
# Group:        System
# Source:       N/A
# URL:          https://github.com/lateralblast
  ...
  if $kernel == "SunOS" and $kernelrelease =~ /10|11/ {
    if $name !~ /\// {
      case $name {
        ...
        /[asr|snmp]-notify/:                              { $service = "svc:/system/fm/${name}:default" }
        "dhcp":                                           { $service = "svc:/network/dhcp/server:ipv4" }
        /devchassis/:                                     { $service = "svc:/system/devchassis:daemon" }
        /texinfo-update|stosreg/:                         { $service = "svc:/application/${name}:default" }
        /puppet:[agent|master]/:                          { $service = "svc:/application/${name}" }
        "mpxio":                                          { $service = "svc:/system/device/mpxio-upgrade:default" }
        "nisd":                                           { $service = "svc:/network/nis/domain:default" }
        /ssh|ipfilter|comsat|ntp|dlmp|ipmievd|ftp$/:      { $service = "svc:/network/${name}:default" }
        /inetd-upgrade/:                                  { $service = "svc:/network/inetd-upgrade:default" }
        /ike$|manual-key/:                                { $service = "svc:/network/ipsec/${name}:default" }
        "ikev2":                                          { $service = "svc:/network/ipsec/ike:ikev2" }
        /[ldap|nis|smb|nfs|sendmail|dns]\/client/:        { $service = "svc:/network/${name}:default" }
        /nlockmgr|cbd|mapid|status|rquota|fedfs-client/:  { $service = "svc:/network/nfs/${name}:default" }
        /spray|rex|rusers|wall|keyserv|rstat|bind/:       { $service = "svc:/network/rpc/${name}:default" }
        /gss|smserver/:                                   { $service = "svc:/network/rpc/${name}:default" }
        ...
        /wusbd|rda|sar|consadm|pools|rcap|vbios/:         { $service = "svc:/system/${name}:default" }
        /name-service-cache|idmap|hostid|console-reset/:  { $service = "svc:/system/${name}:default" }
        /ce-controls|scheduler|cryptosvc/:                { $service = "svc:/system/${name}:default" }
        /zones-monitoring|manifest-import|fmd/:           { $service = "svc:/system/${name}:default" }
        /pkgserv|sysevent|utmp|environment|devfsadm/:     { $service = "svc:/system/${name}:default" }
        /boot-loader-update|hotplug|power|cron/:          { $service = "svc:/system/${name}:default" }
        /logadm-upgrade|resource-mgmt|auditset/:          { $service = "svc:/system/${name}:default" }
        /ca-certificate|picl|fcoe_initiator/:             { $service = "svc:/system/${name}:default" }
        /ocm|initrd|boot-config|zones$|zones-install/:    { $service = "svc:/system/${name}:default" }
        /dumpadm|auditd|console-login|vtdaemon|dbus/:     { $service = "svc:/system/${name}:default" }
        /coreadm|timezone|config-user|keymap/:            { $service = "svc:/system/${name}:default" }
        /netmask|^service$|install|location|ipmp/:        { $service = "svc:/network/${name}:default" }
        /[nfs|smb|dns]\/server/:                          { $service = "svc:/network/${name}:default" }
        /tcsd/:                                           { $service = "svc:/application/security/tcsd:default" }
        /net-snmp|^snmp$/:                                { $service = "svc:/application/management/net-snmp:default" }
        /system-repository|mirror|zones-proxyd/:          { $service = "svc:/application/pkg/${name}:default" }
        ...
        /terma|termb/:                                    { $service = "svc:/system/console-login:${name}" }
        ...
        /relay:[ipv4|ipv6]/:                              { $service = "svc:/network/dhcp/${name}" }
        /slp|ipmon|diagnostics|stlisten/:                 { $service = "svc:/network/${name}:default" }
        /finger|rexec|telnet|loopback/:                   { $service = "svc:/network/${name}:default" }
        /loadbalancer|ilb/:                               { $service = "svc:/network/loadbalancer/ilb:default" }
        /route$|rdisc|ripng|ndp/:                         { $service = "svc:/network/routing/${name}:default" }
        /legacy-routing:[ipv6|ipv4]/:                     { $service = "svc:/network/routing/${name}" }
        /flow|process|task|net$/:                         { $service = "svc:/system/extended-accounting:${name}" }
        /pools\/dynamic/:                                 { $service = "svc:/system/pools/dynamic:default" }
        /snmp-notify|notify-params/:                      { $service = "svc:/system/fm/${name}:default" }
        /global/:                                         { $service = "svc:/system/svc/global:default" }
        /rad\/[local|remote]/:                            { $service = "svc:/system/${name}" }
        /acpi/:                                           { $service = "svc:/platform/i86pc/acpihpd:default" }
        /lpd/:                                            { $service = "svc:/application/cups/in-lpd:default" }
        /eklogin|^klogin|rlogin/:                         { $service = "svc:/network/login:${name}" }
        ...
        /[chargen|discard|echo|time|daytime]:dgram/:      { $service = "svc:/network/${name}" }
        /[chargen|discar|echo|timed|daytime]:stream/:     { $service = "svc:/network/${name}" }
        /stdiscovery|talk|socket-config/:                 { $service = "svc:/network/${name}:default" }
        /smb$|netcfg|datalink-management/:                { $service = "svc:/network/${name}:default" }
        /ip-interface-management|^shell$|npiv_config/:    { $service = "svc:/network/${name}:default" }
        ...
        /root|usr|local|reparse|autofs|rmvolmgr|reparse/: { $service = "svc:/system/filesystem/${name}:default" }
        /ipsecalgs/:                                      { $service = "svc:/network/ipsec/ipsecalgs:default" }
        /boot-archive$|pfexec|rmtmpfiles|system-log/:     { $service = "svc:/system/${name}:default" }
        /index|man/:                                      { $service = "svc:/application/man-index:default" }
        /cleanstart/:                                     { $service = "svc:/system/devchassis:cleanstart" }
        /uucp-lock-cleanup|iptun|routing-setup|inetd/:    { $service = "svc:/network/${name}:default" }
        ...
        /unconfig|devices|name-services|single-user/:     { $service = "svc:/milestone/${name}:default" }
        /multi-user|self-assembly-complete/:              { $service = "svc:/milestone/${name}:default" }
        /input-method-cache|pixbuf-loaders-installer/:    { $service = "svc:/application/desktop-cache/${name}:default" }
        /icon/:                                           { $service = "svc:/application/desktop-cache/icon-cache:default" }
        /identity:[node|domain]/:                         { $service = "svc:/system/${name}" }
        ...
        /shares|ilomconfig-interconnect/:                 { $service = "svc:/network/${name}:default" }
        /vt[0-9]/:                                        { $service = "svc:/system/console-login:${name}" }
        /iscsi\/[target|initator]/:                       { $service = "svc:/network/${name}:default" }
      ...
      if $kernel == "Darwin" {
        if $name !~ /\./ and $name !~ /^[bootps|exec|finger|ftp|login|ntalk]$/ {
          case $name {
            /httpd|apache/:                                   { $service = "org.apache.httpd" }
            /cups/:                                           { $service = "org.cups.${name}" }
            /snmp/:                                           { $service = "org.net-snmp" }
            /openldap|slapd/:                                 { $service = "org.openldap.slapd" }
            /postfix/:                                        { $service = "org.postfix.master" }
            /[G,g]ame[C,c]ontroller/:                         { $service = "com.apple.GameController.gamecontrollerd" }
            /digest-service|kadmin|kcm|kdc|kpasswdd/:         { $service = "com.apple.Kerberos.${name}" }
            /cloudconfigurationd|enroll|starup/:              { $service = "com.apple.ManagedClient" }
            /[R,r]emote[D,d]esktop/:                          { $service = "com.apple.RemoteDesktop.PrivilegeProxy" }
            /[R,r]eport[C,c]rash/:                            { $service = "com.apple.ReportCrash.Root" }
            /TrustEvaluationAgent/:                           { $service = "com.apple.TrustEvaluationAgent.system" }
            /appleseed|fbahelperd/:                           { $service = "com.apple.appleseed.fbahelperd" }
            /coreaudiod/:                                     { $service = "com.apple.audio.coreaudiod" }
            /bsd|dirhelper/:                                  { $service = "com.apple.bsd.dirhelper" }
            /cfprefsd|xpc/:                                   { $service = "com.apple.cfprefsd.xpc.daemon" }
            /AVCAssistant|AppleCameraAssistant/:              { $service = "com.apple.cmio.${name}" }
            /IIDCVideoAssistant|VDCAssistant/:                { $service = "com.apple.cmio.${name}" }
            /iOSScreenCaptureAssistant/:                      { $service = "com.apple.cmio.${name}" }
            /coreduetd/:                                      { $service = "com.apple.coreduetd.osx.plist" }
            /appleevents|launchservicesd/:                    { $service = "com.apple.coreservices.${name}" }
            /appleid|passwordcheck/:                          { $service = "com.apple.coreservices.appleid.passwordcheck" }
            /corestoraged|corestoragehelperd/:                { $service = "com.apple.corestorage.${name}" }
            /distnoted|xpc/:                                  { $service = "com.apple.distnoted.xpc.daemon" }
            /dvdplayback|setregion/:                          { $service = "com.apple.dvdplayback.setregion" }
            /aslmanager/:                                     { $service = "com.apple.emond.aslmanager" }
            /icloud|findmydeviced/:                           { $service = "com.apple.icloud.findmydeviced" }
            /iconservices|iconservicesagent/:                 { $service = "com.apple.iconservices.${name}" }
            /installandsetup|systemmigrationd/:               { $service = "com.apple.installandsetup.systemmigrationd" }
            /LFVTracer/:                                      { $service = "com.apple.loginwindow.LFVTracer" }
            /mdmclient/:                                      { $service = "com.apple.mdmclient.daemon" }
            "mds":                                            { $service = "com.apple.metadata.mds" }
            /index|scan|spindump/:                            { $service = "com.apple.metadata.mds.${name}"}
            /echosvc|lsarpc|mdssvc|netlogon|srvsvc|wkssvc/:   { $service = "com.apple.msrpc.${name}" }
            /auth$|gui/:                                      { $service = "com.apple.netauth.sys.${name}" }
            /nis|ypbind/:                                     { $service = "com.apple.nis.ypbind" }
            /noticeboard|stat/:                               { $service = "com.apple.noticeboard.state" }
            /swd/:                                            { $service = "com.apple.powerd.swd" }
            /admintool|auto/:                                 { $service = "com.apple.preferences.timezone" }
            /printtool/:                                      { $service = "com.apple.preferences.timezone.${name}" }
            /FDERecoveryAgent|agentMain|authhost|syspolicy/:  { $service = "com.apple.security.${name}" }
            /speech/:                                         { $service = "com.apple.speech.speechsynthesisd" }
            /systempreferences/:                              { $service = "com.apple.systempreferences.installer" }
            /apache|httpd/:                                   { $service = "org.apache.httpd" }
            /unmountassistant|sysagent/:                      { $service = "com.apple.unmountassistant.sysagent" }
            /cupsd|cups-lpd/:                                 { $service = "org.cups.${name}" }
            /snmp/:                                           { $service = "org.net-snmp.snmpd" }
            /ntp/:                                            { $service = "org.ntp.ntpd" }
            /ldap|slap/:                                      { $service = "org.openldap.slapd" }
            /postfix/:                                        { $service = "org.postfix.master" }
            default:                                             { $service = "com.apple.${name}"}
          }
        }
        if $name !~ /\// {
